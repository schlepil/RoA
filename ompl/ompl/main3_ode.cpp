...
//Do the integration via gsl
//The dynamics
int fDyn(double t, const double X[], double f[], void *params){
    double ctrl = *(double *)params;

    f[0] = X[1];
    //f[1] = sin(X[0])*(g/l) - X[1]*d/(m*l*l) + 1./(m*l*l)*ctrl;
    //qdd = (obj.m*obj.g*obj.lc*sin(q) - obj.b*qd)/obj.I;
    f[1] = ( ctrl - m*g*lc*sin(X[0]) - b*X[1])/I;
...
int jacDyn(double t, const double X[], double *dfdX, double dfdt[], void *params){
    ...
    gsl_matrix * mJac = &dfdX_mat.matrix;
    //Set the actual jacobian
    /*
    gsl_matrix_set (mJac, 0, 0, 0.0); // d/dX[0] f[0]
    gsl_matrix_set (mJac, 0, 1, 1.0); // d/dX[1] f[0]
    gsl_matrix_set (mJac, 1, 0, cos(X[0])*(g/l)); // d/dX[0] f[1]
    gsl_matrix_set (mJac, 1, 1, -d/(m*l*l)); // d/dX[1] f[1]
    */
    gsl_matrix_set (mJac, 0, 0, 0.0); // d/dX[0] f[0]
    gsl_matrix_set (mJac, 0, 1, 1.0); // d/dX[1] f[0]
    gsl_matrix_set (mJac, 1, 0, -cos(X[0])*m*g); // d/dX[0] f[1]
    gsl_matrix_set (mJac, 1, 1, -b/I); // d/dX[1] f[1]
...
gsl_odeiv_system sys = {fDyn, jacDyn, 2, &ctrlODE};
//Define a integration function
void integrateInvPend(double X[], double t, double tEnd){
...
void propagate(const ompl::base::State *start, const ompl::control::Control *control, const double duration, ompl::base::State *result){
    ...
    //Use gsl to propagate

    double X[2] = { ainvPendState->values[0], ainvPendState->values[1] };

    //cout << ctrlODE << endl;
    ctrlODE = (double) control->as<ompl::control::RealVectorControlSpace::ControlType>()->values[0];

    integrateInvPend(X, 0.0, duration);

    result->as<ompl::base::RealVectorStateSpace::StateType>()->values[0] = X[0];
    result->as<ompl::base::RealVectorStateSpace::StateType>()->values[1] = X[1];
}
...
int main()
{
    ...
    if (solved){
        ...
        ofstream resFile;
        resFile.open("invPendSwingUpNew.txt");
