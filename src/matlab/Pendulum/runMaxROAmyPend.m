function runMaxROAmyPend

p = PendulumPlant;
p.setInputLimits(-3,3)
% Set input limits
...
x0 = Point(p.getStateFrame,[pi;0]);
u0 = Point(p.getInputFrame,0);
sys = taylorApprox(p,0,x0,u0,3);

% Options for control design
options = struct();
options.controller_deg = 3%1; % Degree of polynomial controller to search for
...
fInt = @(t,x) p.dynamics(t, x, max(min(K(x-x0A),3),-3));
figure(); hold all;
if 0
    for k = 1:size(xinit,2)
        [tSol, xSol] = ode45(fInt, [0,4], xinit(:,k)+x0A);
        plot(xSol(:,1), xSol(:,2))
